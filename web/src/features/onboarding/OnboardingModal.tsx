...
export const OnboardingModal: React.FC = () => {
  const { isOnboardingOpen, closeOnboarding } = useModals()
  ...
  const slides = [
    {
      ...
      description:
        'In a Paluwagan, a trusted group of friends, colleagues, or family members pool a fixed amount of money at regular intervals (weekly or semi-monthly).',
    ...
    },
    {
      ...
      description:
        'Each cycle, one member is designated to receive the entire pooled pot. Slots can be chosen by lottery, consensus, or seniority. The cycle continues until everyone receives their pot once.',
    ...
    },
    {
      ...
      description:
        'Set custom deadlines matching payrolls (15th & 30th or weekly). Upload payment reference slips from GCash, Maya, or Bank transfers for instant organizer sign-off.',
    ...
    },
    {
      ...
      description:
        'Invite members via link or direct contact. Each completed cycle and timely contribution boosts your verified Trust Score (up to 100%), unlocking higher capital circles.',
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 pt-5 pb-2">
          ...
          >
            {slides.map((_, idx) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all duration-300 focus:outline-hidden focus-visible:ring-1 focus-visible:ring-[#F5B800] ${
                  idx === currentSlide
                    ? 'w-7 bg-[#F5B800]'
          ...
          <button
            ...
            className="text-xs font-bold text-neutral-500 hover:text-neutral-950 dark:text-neutral-400 dark:hover:text-white transition-colors cursor-pointer px-2 py-1 rounded-md focus:outline-hidden focus-visible:ring-1 focus-visible:ring-[#F5B800]"
          >
            Skip Guide
          </button>
        </div>
        ...
        <div className="p-6 md:p-8 flex-1 min-h-[320px] flex flex-col justify-center">
          <AnimatePresence mode="wait">
            ...
            >
              {/* Feature Icon */}
              <div
                className={`w-16 h-16 rounded-2xl ${current.bgBadge} flex items-center justify-center ${current.color} mb-5 shadow-xs`}
              >
              ...
              <div className="mt-5 px-3 py-2 rounded-xl bg-neutral-50 dark:bg-neutral-800/60 border border-neutral-200 dark:border-neutral-800 text-xs font-bold text-neutral-900 dark:text-neutral-100 flex items-center gap-2">
                <Check
                  className="w-4 h-4 text-[#F5B800] shrink-0 stroke-[3]"
                  aria-hidden="true"
                />
        ...
        {/* Bottom Actions Bar */}
        <div className="px-6 py-4 bg-neutral-50 dark:bg-neutral-900/60 border-t border-neutral-200 dark:border-neutral-800 flex items-center justify-between">
          <button
            id="onboarding-prev-btn"
            type="button"
            onClick={handlePrev}
            disabled={currentSlide === 0}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all focus:outline-hidden focus-visible:ring-1 focus-visible:ring-[#F5B800] ${
              currentSlide === 0
