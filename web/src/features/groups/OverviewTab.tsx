...
export const OverviewTab: React.FC = () => {
  ...
  const { openNewGroup, openContribution } = useModals()
  const navigate = useNavigate()

  const [viewMode, setViewMode] = useState<'bento' | 'table'>('bento')
  const [statusFilter, setStatusFilter] = useState<'all' | 'due'>('all')

  const primaryGroup: PaluwaganGroup | null =
    displayedGroups.length > 0 ? (displayedGroups[0] ?? null) : null
  const currentRecipient =
    primaryGroup?.members.find((m) => m.status === 'current_pot') ?? null

  const totalPooledFunds = displayedGroups.reduce(
    (acc, g) => acc + g.potAmount,
    0,
  )
  const totalContributionsPaid = displayedGroups.reduce((acc, g) => {
    const user = g.members.find((m) => m.isCurrentUser)
    return acc + (user?.currentCyclePaid ? g.contributionAmount : 0)
  }, 0)

  const pendingContributions = displayedGroups.filter((g) => {
  ...
  return (
    <div className="space-y-6">
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
                ...
                <span className="text-xs font-extrabold uppercase tracking-wider text-neutral-900 bg-black/10 px-2.5 py-1 rounded-md">
                  Round {primaryGroup.currentRound} of{' '}
                  {primaryGroup.totalRounds}
                </span>
              </div>
              ...
              <div className="mt-3">
                ...
                <p className="text-xs font-semibold text-neutral-900 mt-2">
                  ...
                  <strong>{primaryGroup.nextPayoutDate}</strong> (Upcoming
                  round)
                </p>
              </div>
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-sm font-black text-white truncate">
                        ...
                        {currentRecipient.isCurrentUser && '(You!)'}
                      </p>
                    </div>
                  </div>
            ...
            <div className="mt-6 pt-4 border-t border-black/15 flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-1.5 text-xs font-bold text-neutral-900">
                ...
                <span className="bg-black/10 px-2 py-0.5 rounded">
                  ₱{primaryGroup.contributionAmount.toLocaleString()} / member
                </span>
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3.5 py-2 rounded-lg text-xs font-bold text-neutral-950 bg-white/90 hover:bg-white active:bg-white/80 transition-all cursor-pointer shadow-xs focus:outline-hidden focus-visible:ring-2 focus-visible:ring-neutral-950"
                >
                  View Rotation Order →
                </button>
                <button
                  type="button"
                  onClick={() => openContribution(primaryGroup)}
                  className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-bold text-white bg-neutral-950 hover:bg-neutral-850 active:bg-neutral-900 transition-all cursor-pointer shadow-xs focus:outline-hidden focus-visible:ring-2 focus-visible:ring-white"
                >
        ...
        <div className="lg:col-span-4 rounded-2xl bg-white dark:bg-[#16181D] border border-neutral-200 dark:border-neutral-800 p-6 flex flex-col justify-between shadow-xs">
            ...
            <div className="mt-5 space-y-2.5 pt-4 border-t border-neutral-100 dark:border-neutral-800 text-xs">
              <div className="flex items-center justify-between">
                ...
                <span className="font-bold text-neutral-900 dark:text-neutral-100">
                  {currentUser.completedCycles} Cycles (100% On-Time)
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-neutral-500 dark:text-neutral-400 font-medium">
              ...
              <div className="flex items-center justify-between">
                ...
                >
                  {pendingContributions.length > 0
                    ? `${pendingContributions.length} Hulog Due Soon`
          ...
          <div className="mt-6 pt-3 border-t border-neutral-100 dark:border-neutral-800">
            <button
              ...
              className="w-full py-2 px-3 rounded-lg text-xs font-bold text-neutral-950 bg-[#F5B800] hover:bg-[#E0A700] active:bg-[#C79200] transition-colors flex items-center justify-center gap-1.5 cursor-pointer shadow-xs focus:outline-hidden focus-visible:ring-2 focus-visible:ring-[#F5B800]"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-4 rounded-xl bg-white dark:bg-[#16181D] border border-neutral-200 dark:border-neutral-800 shadow-xs">
          ...
          <span className="text-[11px] font-semibold text-neutral-500 mt-1 block">
            Across {displayedGroups.length} circles
          </span>
        </div>
        ...
        <div className="p-4 rounded-xl bg-white dark:bg-[#16181D] border border-neutral-200 dark:border-neutral-800 shadow-xs">
          ...
          <span className="text-[11px] font-semibold text-[#8A6300] dark:text-[#FACC15] mt-1 block truncate">
            {primaryGroup?.name ?? 'No active group'}
          </span>
        </div>
        ...
        <div className="p-4 rounded-xl bg-white dark:bg-[#16181D] border border-neutral-200 dark:border-neutral-800 shadow-xs">
          ...
          <p className="text-2xl font-black tracking-tight text-neutral-950 dark:text-white mt-1">
            {displayedGroups.reduce((acc, g) => acc + g.members.length, 0)}{' '}
            Members
          </p>
          <span className="text-[11px] font-semibold text-neutral-500 mt-1 block">
      ...
      {/* 3. ENTERPRISE TABLE & BENTO CARD HYBRID */}
      <div className="rounded-2xl bg-white dark:bg-[#16181D] border border-neutral-200 dark:border-neutral-800 p-5 shadow-xs space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-neutral-100 dark:border-neutral-800">
          <div>
            ...
            <p className="text-xs text-neutral-500 dark:text-neutral-400">
              Enterprise management of active rotation cycles, disbursement
              timelines, and member contribution compliance.
            </p>
          </div>
        ...
        {filteredGroups.length === 0 ? (
          <EmptyState
            icon={<PiggyBank className="w-10 h-10 stroke-1" />}
            title="No circles found"
            description={
              statusFilter === 'due'
                ? 'You have no outstanding contributions due right now. Excellent track record!'
                : 'No Paluwagan circles match your search query.'
            }
            actionLabel={
              statusFilter === 'due' ? undefined : '+ Create New Circle'
            }
            onAction={statusFilter === 'due' ? undefined : openNewGroup}
          />
        ) : viewMode === 'table' ? (
                    ...
                    >
                      ...
                      <td className="py-3 px-3 min-w-[130px]">
                        ...
                        <div className="w-full bg-neutral-200 dark:bg-neutral-700 h-1.5 rounded-full overflow-hidden">
                          <div
                            className="bg-[#F5B800] h-full rounded-full"
                      ...
                      <td className="py-3 px-3 text-right">
                        <button
                          ...
                          className="px-2.5 py-1 text-xs font-bold rounded text-neutral-950 bg-[#F5B800] hover:bg-[#E0A700] transition-colors cursor-pointer"
                        >
                          Open →
                        </button>
                      </td>
