...
export const ContributionModal: React.FC = () => {
  const { isContributionOpen, activeContributionGroup, closeContribution } =
    useModals()
  const { submitContribution } = useGroups()

  const [selectedMethod, setSelectedMethod] = useState<
    'GCash' | 'Maya' | 'BPI Online' | 'BDO Bank'
  >('GCash')
  ...
  if (!isContributionOpen || !activeContributionGroup) return null
  ...
  const paymentMethods = [
    {
      ...
      name: 'GCash',
      icon: Smartphone,
      account: '0917 555 8921 (Maria Santos)',
    },
    {
      id: 'Maya',
      name: 'Maya Wallet',
    ...
    },
    {
      ...
      name: 'BPI Online',
      icon: Building2,
      account: 'Acct # 4410-9281-99 (TrackPal)',
    },
    {
      id: 'BDO Bank',
      name: 'BDO Unibank',
      icon: Building2,
      account: 'Acct # 0019-3829-11 (TrackPal)',
    },
  ]
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        submitContribution(
          activeContributionGroup.id,
          selectedMethod,
          finalRef,
          activeContributionGroup.contributionAmount,
        )
        ...
        <div className="px-6 pt-6 pb-4 border-b border-neutral-100 dark:border-neutral-800">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded bg-[#F5B800] text-neutral-950">
              Hulog Submission • Round #{activeContributionGroup.currentRound}
            </span>
          </div>
          <h2
            id="contribution-modal-title"
            className="text-2xl font-black text-neutral-950 dark:text-white tracking-tight mt-2"
          >
            Submit Contribution
          </h2>
          <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-1">
            {activeContributionGroup.name} • Due amount:{' '}
            <strong className="text-neutral-950 dark:text-white font-bold">
              ₱{activeContributionGroup.contributionAmount.toLocaleString()}
            </strong>
          </p>
        </div>
        ...
        {showSuccess ? (
          ...
          >
            ...
            <p className="text-xs text-neutral-500 max-w-sm">
              ...
              {activeContributionGroup.contributionAmount.toLocaleString()} has
              been verified and logged into the circle rotation ledger.
            </p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="p-6 space-y-5">
                ...
                {paymentMethods.map((pm) => {
                  ...
                  return (
                    <button
                      ...
                      className={`p-3 rounded-xl border text-left transition-all cursor-pointer focus:outline-hidden focus-visible:ring-2 focus-visible:ring-[#F5B800] ${
                        isSelected
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <Icon
                          className={`w-4 h-4 ${isSelected ? 'text-neutral-950 dark:text-[#F5B800]' : 'text-neutral-400'}`}
                          aria-hidden="true"
                        />
            ...
            <div>
              <label
                htmlFor="contribution-ref-number-input"
            ...
            {/* File Upload */}
            <div>
              <span className="block text-xs font-bold text-neutral-900 dark:text-neutral-100 mb-1">
                Attach Proof of Transfer (Receipt Screenshot)
              </span>
              <label
                htmlFor="receipt-upload"
                className="flex flex-col items-center justify-center p-4 border-2 border-dashed border-neutral-300 dark:border-neutral-700 rounded-xl hover:border-[#F5B800] bg-neutral-50 dark:bg-neutral-800/50 cursor-pointer transition-colors"
              >
                <UploadCloud
