...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (validate()) {
      onLoginSuccess(isSignUp ? fullName : (email.split('@')[0] || 'Member'), email);
    }
  };

  const handleDemoSignIn = (role: 'Maria Santos (Organizer)' | 'Juan Dela Cruz (Saver)') => {
    if (role.startsWith('Maria')) {
      onLoginSuccess('Maria Santos', 'maria.santos@gmail.com');
    } else {
      onLoginSuccess('Juan Dela Cruz', 'juan.delacruz@outlook.com');
    }
  };
      ...
      >
        ...
        <div className="px-6 space-y-2">
          ...
          <button
            ...
            type="button"
            onClick={() => onLoginSuccess('Juan Dela Cruz', 'juan.delacruz@outlook.com')}
            className="w-full flex items-center justify-center gap-3 py-2.5 px-4 rounded-xl border border-neutral-200 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-800 hover:bg-neutral-100 dark:hover:bg-neutral-750 text-xs font-bold text-neutral-950 dark:text-white transition-all shadow-xs cursor-pointer"
          >
          ...
          <button
            ...
            type="button"
            onClick={() => onLoginSuccess('Maria Santos', 'maria.santos@gmail.com')}
            className="w-full flex items-center justify-center gap-3 py-2.5 px-4 rounded-xl border border-neutral-200 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-800 hover:bg-neutral-100 dark:hover:bg-neutral-750 text-xs font-bold text-neutral-950 dark:text-white transition-all shadow-xs cursor-pointer"
          >
        ...
        <form onSubmit={handleSubmit} className="px-6 pb-6 space-y-4">
          ...
          <button
            id="auth-submit-btn"
            type="submit"
            className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg text-xs font-bold text-neutral-950 bg-[#F5B800] hover:bg-[#E0A700] active:bg-[#C79200] shadow-xs transition-all cursor-pointer mt-2"
          >
          ...
          <div className="text-center pt-2">
            ...
            >
              {isSignUp ? (
                <>Already have an account? <span className="text-neutral-950 dark:text-[#F5B800] font-bold">Sign in</span></>
              ) : (
                <>Don't have an account? <span className="text-neutral-950 dark:text-[#F5B800] font-bold">Register</span></>
              )}
            </button>
          </div>

          {/* Quick Demo Access Bento */}
          <div className="mt-4 p-3 rounded-xl bg-neutral-50 dark:bg-neutral-850 border border-neutral-200 dark:border-neutral-800">
