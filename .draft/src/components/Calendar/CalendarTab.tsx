...
interface CalendarTabProps {
  ...
  onOpenContribution: (groupId: string) => void;
  darkMode: boolean;
}
...
export const CalendarTab: React.FC<CalendarTabProps> = ({
  ...
  const [selectedFlyoutEvent, setSelectedFlyoutEvent] = useState<CalendarEvent | null>(null);

  // Month info (September 2026)
  const daysInSeptember = 30;
  // Sept 1, 2026 is a Tuesday (index 2: Sun=0, Mon=1, Tue=2)
  const startingDayOffset = 2; 
  ...
  const handleDayClick = (day: number) => {
    ...
    if (dayEvents.length > 0) {
      setSelectedFlyoutEvent(dayEvents[0]);
    } else {
      setSelectedFlyoutEvent(null);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        {/* The Grid (takes 8 cols on desktop) */}
        <div className="lg:col-span-8 bg-white dark:bg-[#16181D] border border-neutral-200 dark:border-neutral-800 rounded-2xl p-4 sm:p-5 shadow-xs">
          ...
          <div className="grid grid-cols-7 gap-2 pt-3">
            ...
            {Array.from({ length: startingDayOffset }).map((_, i) => (
              <div
                key={`empty-${i}`}
                className="h-20 sm:h-24 p-1.5 rounded-xl bg-neutral-50/50 dark:bg-neutral-850/40 border border-transparent opacity-30"
              />
            ))}
            ...
            {daysArray.map((day) => {
              ...
              return (
                <div
                  ...
                  className={`h-20 sm:h-24 p-2 rounded-xl border transition-all cursor-pointer flex flex-col justify-between group ${
                    isSelected
        ...
        {/* Right Column: Selected Date Schedule & Action Flyout (Span 4) */}
        <div className="lg:col-span-4 bg-white dark:bg-[#16181D] border border-neutral-200 dark:border-neutral-800 rounded-2xl p-5 shadow-xs flex flex-col justify-between">
          <div>
            ...
            {/* If events exist for this date */}
            {selectedDate && getEventsForDay(Number(selectedDate.split('-')[2])).length > 0 ? (
              <div className="space-y-3">
                {getEventsForDay(Number(selectedDate.split('-')[2])).map((ev) => (
                  <div
                    key={ev.id}
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className={`text-[10px] font-black px-2 py-0.5 rounded uppercase ${
                        ...
                        {ev.type === 'payout_day' ? 'Payout Distribution' : 'Hulog Deadline'}
                      </span>
                    ...
                    {ev.recipientName && (
                      <p className="text-[11px] text-neutral-500 dark:text-neutral-400">
                        Turn Recipient: <strong className="text-neutral-800 dark:text-neutral-200">{ev.recipientName}</strong>
                      </p>
                    )}

                    {/* Action Button */}
                    <button
                      onClick={() => onOpenContribution(ev.groupId)}
                      className="w-full flex items-center justify-center gap-1.5 py-2 px-3 rounded-lg text-xs font-bold text-neutral-950 bg-[#F5B800] hover:bg-[#E0A700] active:bg-[#C79200] transition-all cursor-pointer shadow-xs"
                    >
                      <CreditCard className="w-3.5 h-3.5 stroke-[2.5]" />
                      <span>{ev.type === 'payout_day' ? 'View Circle Ledger' : 'Submit Hulog'}</span>
                    </button>
                  </div>
                ))}
              </div>
              ...
              <div className="py-12 text-center text-neutral-400 space-y-2">
                <Clock className="w-8 h-8 mx-auto opacity-40 text-neutral-400" />
                <p className="text-xs font-semibold">No scheduled deadlines or payouts on this day.</p>
              </div>
            )}
          </div>
          ...
          <div className="pt-4 border-t border-neutral-100 dark:border-neutral-800 text-[11px] text-neutral-500 dark:text-neutral-400 flex items-center gap-2 mt-4">
            <Sparkles className="w-4 h-4 text-[#8A6300] dark:text-[#FACC15] shrink-0" />
            <span>Automatic reminders notify members 48 hours before collection cutoffs.</span>
          </div>
        </div>
      </div>
    </div>
