...
import { PaluwaganGroup, PaluwaganMember } from '../../types';

interface GroupDetailTabProps {
  group: PaluwaganGroup;
  onBack: () => void;
  onSubmitContribution: (group: PaluwaganGroup) => void;
  onInviteFriend: (group: PaluwaganGroup) => void;
  onToggleMemberPaid: (groupId: string, memberId: string) => void;
  darkMode: boolean;
}

export const GroupDetailTab: React.FC<GroupDetailTabProps> = ({
  group,
  onBack,
  ...
  const [activeSubTab, setActiveSubTab] = useState<'rotation' | 'current-cycle' | 'rules'>('rotation');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            onClick={() => onInviteFriend(group)}
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-xs font-bold text-neutral-800 dark:text-neutral-200 bg-white dark:bg-[#16181D] border border-neutral-200 dark:border-neutral-800 hover:bg-neutral-50 dark:hover:bg-neutral-800 transition-all cursor-pointer shadow-xs"
          >
            <UserPlus className="w-3.5 h-3.5 text-[#8A6300] dark:text-[#FACC15]" />
            <span>Invite Member</span>
          </button>

          <button
            id="group-detail-submit-btn"
            onClick={() => onSubmitContribution(group)}
            className="flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold text-neutral-950 bg-[#F5B800] hover:bg-[#E0A700] active:bg-[#C79200] shadow-xs hover:shadow transition-all cursor-pointer"
          >
            <CreditCard className="w-3.5 h-3.5 stroke-[2.5]" />
            <span>Submit Hulog (₱{group.contributionAmount.toLocaleString()})</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-8 rounded-2xl bg-white dark:bg-[#16181D] border border-neutral-200 dark:border-neutral-800 p-6 flex flex-col justify-between shadow-xs">
          ...
          <div className="mt-6 pt-4 border-t border-neutral-100 dark:border-neutral-800 flex flex-wrap items-center gap-6 text-xs text-neutral-500">
            <div>
              <span className="text-[10px] uppercase font-bold text-neutral-400 block">Organizer</span>
              <strong className="text-neutral-900 dark:text-neutral-100">{group.organizer.name}</strong>
            </div>
            <div>
              <span className="text-[10px] uppercase font-bold text-neutral-400 block">Started Date</span>
              <strong className="text-neutral-900 dark:text-neutral-100">{group.startDate}</strong>
            </div>
            <div>
              <span className="text-[10px] uppercase font-bold text-neutral-400 block">Slots / Members</span>
              <strong className="text-neutral-900 dark:text-neutral-100">{group.members.length} Members</strong>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 rounded-2xl bg-[#F5B800] text-neutral-950 p-6 flex flex-col justify-between shadow-xs">
          <div>
            <div className="flex items-center justify-between">
              ...
              <PiggyBank className="w-5 h-5 text-neutral-950" />
            </div>
            ...
            <p className="text-xs font-bold text-neutral-900 mt-2">
              Share: ₱{group.contributionAmount.toLocaleString()} / member per round
            </p>
          </div>
          ...
          <div className="mt-6 pt-4 border-t border-black/15">
            ...
            <p className="text-[11px] font-semibold text-neutral-800 mt-1.5">
              {paidCount} of {group.members.length} members paid (₱{currentCollectedAmount.toLocaleString()})
            </p>
          </div>
        </div>
      </div>

      {/* SUB-TABS: Ant Design Segmented Pill Bar */}
      <div className="flex items-center gap-1.5 bg-neutral-100 dark:bg-neutral-800/80 p-1 rounded-xl w-fit">
        <button
          id="tab-btn-rotation"
      ...
      {/* TAB CONTENT 1: ANT DESIGN DATA TABLE - PAYOUT ROTATION ORDER */}
      {activeSubTab === 'rotation' && (
          ...
          <div className="p-4 sm:p-5 border-b border-neutral-100 dark:border-neutral-800 flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-neutral-500 dark:text-neutral-400">
                Fixed slot assignments. Each member receives the full ₱{group.potAmount.toLocaleString()} pot on their turn.
              </p>
            </div>
        ...
        <div className="rounded-2xl bg-white dark:bg-[#16181D] border border-neutral-200 dark:border-neutral-800 p-5 shadow-xs space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-neutral-100 dark:border-neutral-800">
            <div>
              ...
              <p className="text-xs text-neutral-500 dark:text-neutral-400">
                Automated ledger for member contributions (₱{group.contributionAmount.toLocaleString()} each) due this round.
              </p>
            </div>
            ...
            {group.members.map((member) => (
              ...
              >
                  ...
                  <div className="min-w-0">
                    ...
                    {member.currentCyclePaid ? (
                      <p className="text-[11px] text-emerald-600 dark:text-emerald-400 flex items-center gap-1 mt-0.5">
                        ...
                        <span>Paid via {member.paymentMethod || 'Online'} • Ref: {member.paymentRef || 'TXN-AUTO'}</span>
                      </p>
                    ) : (
                      <p className="text-[11px] text-amber-600 dark:text-amber-400 flex items-center gap-1 mt-0.5">
                        <AlertCircle className="w-3 h-3" />
                        <span>Awaiting ₱{group.contributionAmount.toLocaleString()} hulog transfer</span>
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2.5 sm:self-center">
                  <button
                    onClick={() => onToggleMemberPaid(group.id, member.id)}
                  ...
                  {!member.currentCyclePaid && (
                    ...
                    >
                      {nudgeSuccessMember === member.id ? (
                        <span className="text-[11px] text-[#8A6300] dark:text-[#FACC15] font-bold">Nudge Sent!</span>
                      ) : (
      ...
      {activeSubTab === 'rules' && (
        <div className="rounded-2xl bg-white dark:bg-[#16181D] border border-neutral-200 dark:border-neutral-800 p-6 shadow-xs space-y-4">
          <div className="flex items-center gap-2 pb-3 border-b border-neutral-100 dark:border-neutral-800">
            <Info className="w-4 h-4 text-[#8A6300] dark:text-[#FACC15]" />
          ...
          <div className="space-y-3 text-xs text-neutral-600 dark:text-neutral-300">
            {group.rules.map((rule, idx) => (
              <div key={idx} className="flex items-start gap-3 p-3.5 rounded-xl bg-neutral-50 dark:bg-neutral-800/60">
            ...
            <div className="p-4 rounded-xl bg-[#F5B800]/10 border border-[#F5B800]/30 text-neutral-950 dark:text-white text-xs">
              <strong className="block font-black mb-1">TrackPal Dispute Protection Guarantee</strong>
              All transfers are logged with immutable cryptographic timestamps and payment slip references. Failure to submit contributions results in immediate trust score deduction.
            </div>
          </div>
        </div>
      )}
